#A biblioteta resquests deve ser importada para trabalhar com requisições
import requests

#O Tkinter é utilizado para criação de janelas via Python
from tkinter import *

def pegar_cotacoes():
    requisicao = requests.get("https://economia.awesomeapi.com.br/last/USD-BRL,EUR-BRL,BTC-BRL")

    requisicao_dic = requisicao.json()

    cotacao_dolar = requisicao_dic['USDBRL']['bid']
    cotacao_euro = requisicao_dic['EURBRL']['bid']
    cotacao_btc = requisicao_dic['BTCBRL']['bid']

    texto = f'''
    Dólar: {cotacao_dolar}
    Euro: {cotacao_euro}
    BTC: {cotacao_btc}'''

    texto_cotacoes["text"] = texto

#Como já foi importado a biblioteca Tkinter em sua totalidade, use o Tk() para poder utilizar ela para criaçã de janelas
janela = Tk()
janela.title("Cotação Moedas")
janela.geometry("350x300")

texto_orientacao = Label(janela, text="Clique no botão para ver as cotações das moedas")
texto_orientacao.grid(column=0, row=0, padx=10, pady=10)

texto_orientacao2 = Label(janela, text="Clique aqui")
texto_orientacao2.grid(column=0, row=1, padx=10, pady=10)

botao = Button(janela, text="Buscar cotações", command=pegar_cotacoes)
botao.grid(column=0, row=2, padx=10, pady=10)

texto_cotacoes = Label(janela, text="")
texto_cotacoes.grid(column=0, row=3, padx=10, pady=10)

#Use sempre o janela.main.loop() para manter a janela ativa
janela.mainloop()